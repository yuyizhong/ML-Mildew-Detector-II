...
def show():
    ...
    st.image(
        labels_distribution,
        caption='Labels Distribution among Train, Validation and Test Sets'
    )
    st.write("---")
    ...
    st.info(f'''
        ...
        end up with loss nearly to zero and accuracy close to 1
        (train_loss: 0.0160 - accuracy: 0.9942,
        val_loss: 0.0107 - val_accuracy: 1.0000). ''')

    model_evaluation = plt.imread(
        f"outputs/{version}/model_training_metrics3.png"
    )
    ...
    evaluation_data = pd.DataFrame(
        joblib.load(f'outputs/{version}/mildew_model_evaluation3.pkl'),
        index=['Loss', 'Accuracy']
    )

    col2.dataframe(evaluation_data, height=200)

    model_evaluation_bar = plt.imread(
        f"outputs/{version}/model_evaluation_metrics.png"
    )

    col1.image(model_evaluation_bar)
    st.write(
        '<style>div.row-widget.stImage {height: 400px;}</style>',
    ...
    st.image(cm_test,
             caption='Confusion matrix - test data', use_column_width=True
             )

    st.success(
        'The classification reports from model evaluation on test data.'
    )
